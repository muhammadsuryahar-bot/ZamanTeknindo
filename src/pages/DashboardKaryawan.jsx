...
const dialStyles = {
  ...
  rentangLabel: { display: "block", marginBottom: 2, fontSize: 9.5, color: warna.tintaSamar },
  rentangValue: { display: "block", fontFamily: font.mono, fontSize: 11.5, color: warna.tinta },
  garisRentang: { height: 6, position: "relative", marginBottom: 2, borderRadius: 999, background: warna.garis, overflow: "hidden" },
  progressRentang: { display: "block", height: "100%", borderRadius: 999, minWidth: 0, maxWidth: "100%" },
};
...
export default function DashboardKaryawan({ pengguna, onLogout }) {
  ...
  async function cobaSinkronAntrian({ refreshStatus = true } = {}) {
    try {
      ...
      setPesanSinkronisasi("");
      const hasil = await sinkronkanAntrian({ apiUrl: API_URL, getToken, penggunaId: pengguna.id });
  ...
  async function ambilStatusHariIni({ pertahankanVerifikasiSaatFallback = false } = {}) {
    ...
    if (!navigator.onLine) {
      ...
      if (!berhasilPakaiCache && mountedRef.current) {
        ...
        setPesan("Koneksi internet tidak tersedia dan status absensi hari ini belum pernah tersimpan di perangkat. Hubungkan internet untuk memuat status absensi.");
      }
